...
class Tuile extends JButton implements MouseListener {
	...
	public void mouseClicked(MouseEvent arg0) {
		...
		case "Paquet":
			if ( event == MouseEvent.BUTTON1 && tuile.CodeCarte.equals("Paquet")) {
				...
				if (!retOk) msg = "Main est complète ou nombre max de cartes tirées atteint!";
		...
		default:
				...
				if( tuile.Emplacement == PANEL_JEU &&  tuile.CodeCarte =="") {
					if(event == MouseEvent.BUTTON1 &&  tuile.CodeCarte =="") {
						...
						if (!retOk) msg = "Encerclement ou pas de murs disponibles ou opération interdite!";
						tuile.Frame.AfficherGrilleHaut(1,null);
						tuile.Frame.AfficherGrillesBas();
					}
				...
				if(tuile.Emplacement == PANEL_PROGRAMME && event == MouseEvent.BUTTON1  &&  tuile.CodeCarte.startsWith("T")) {
					...
					if (newKey == "Erreur") msg = "Erreur d'exécution ou programme vide!";
					if (newKey == "FinManche") msg = "Fin de la manche!";
					if (newKey == "FinPartie") msg = "Fin de la partie!";
					if (newKey == "Impossible") msg = "Impossible d'exécuter, essayez au prochain tour";
	...
	public void mouseEntered(MouseEvent arg0) {
		Tuile tuile = (Tuile) arg0.getSource();
		String msg = "Key:" + tuile.Key+", Code:"+tuile.CodeCarte;

		switch (tuile.CodeCarte) {
		case "Paquet":
			msg += ", Clic pour tirer une carte";
			msg += " / Nb="+ tuile.Partie.GetNbCartePaquet();
			break; 
		case "Defense":
			break; 
		case "Main":
			msg += ", Clic droit pour defausser toute la main";
			break; 
		default:
			//grille du haut
			...
			if( tuile.Emplacement == PANEL_JEU &&  tuile.CodeCarte =="") {
					//case vide
					msg += ", clic droit/gauche pour déposer un mur";
			}
			//defense
			if( tuile.Emplacement == PANEL_DEFENSE &&  tuile.CodeCarte !="") {
				if(tuile.CodeCarte.startsWith("W")) {
					msg += ", clic gauche dans la grille pour déposer ce mur";
				}
				if(tuile.CodeCarte.startsWith("I")) {
					msg += ", clic droit dans la grille pour déposer ce mur";
				}
			}
			//main
			if( tuile.Emplacement == PANEL_MAIN &&  tuile.CodeCarte !="") {
				msg += ", clic pour ajouter la carte au programme & Clic droit pour défausser la carte";
			}
			//programme
			if( tuile.Emplacement == PANEL_PROGRAMME &&  tuile.CodeCarte.startsWith("T") ) {
				msg += ", Nom:"+Partie.GetNomJoueurCourant();
				msg += ", Clic pour exécuter le programme, clic droit pour passer le tour";
...
public class RobotTurtlesInterface implements ActionListener {
	...
	RobotTurtlesInterface(RobotTurtlesPartie partie) {
		...
		panelGauche.add(panelResultat, BorderLayout.EAST);
	...
	public void AfficherGrilleHaut(int mode, String[][] grille) {
		...
		// emplacement 1 = Grille de jeu et 2=Grille joueur
		if (mode == 1) AfficherGrille(Partie.GetGrilleDeJeu(),PanelGrille,1);
	...
	private void AfficherGrilleDuJoueurTest(int mode) {
		...
		if(mode == 2) 
		{
			...
			for (int col = 0; col < grillePaquetDeCartes[0].length; col++)
			{
				int i = col / 8;
				int j = col % 8;
	...
	private void AjusterPanelResultat(int index) {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (sourceAction == comboNbJoueur) {
			this.AjusterPanelResultat(comboNbJoueur.getSelectedIndex());
			//this.SetLangue();
		}
