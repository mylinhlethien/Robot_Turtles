...
class Joueur {
	...
	public String[][] GetGrilleProgramme(int mode){
		String[][] grille;
		if(mode == 1) grille= GrilleProgramme; //  mode=1 afficher les cartes en mode clair et mode = 2 afficher les cartes face cachée
		else {
	...
	private void ExecuterProcedureAvancer() {
		...
		if(newLigTortue < 0 || newLigTortue > 7 || newColTortue < 0 || newColTortue > 7) {
			...
			if(nouvelleCase.startsWith("R")) {// un joyau gangé ! youpi !!
				...
				//si carambolage avec une autre tortue/les deux tortues retournent à leur position initiale
				GrilleDeJeu[ligTortue][colTortue]="";
