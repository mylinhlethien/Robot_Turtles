...
class Joueur {
	...
	public int NbCarteJouees; // nb de carte jouées, sert pour mélanger la defausse
	...
	public String[][] GetGrilleProgramme(int mode){
		String[][] grille;
		if(mode == 1) grille= GrilleProgramme; //  mode=1 afficher les cartes en mode clair et mode = 2 afficher les cartes face cach�e
		else {
	...
	public void NouveauJoyau(String codeCarte,int nbJoueur, int nbGagnant) {
		...
		//nb de joyaux =nb points gagn�es
	...
	public void MelangerInstructions(){
		// nombre al�atoire entre 2 et 37
	...
	public boolean InsererCarteAuProgramme(String codeCarte)
	{
		...
		SetActionDuTour(1); //compl�ter programme
		this.CreerGrilles();
		return true;
	}
	public boolean AnnulerLeProgramme()
	{		
		//annulation apr�s ex�cution
...
public class RobotTurtlesPartie {
	...
	public void Initialiser(int nbManche, int nbJoueur, int modeDeJeux)
	{
		...
		{"B", "./images/cards/bleueCard.png"}, // avancer
		{"P", "./images/cards/purpleCard.png"}, // tourner � droite
		{"Y", "./images/cards/yellowCard.png"}, // tourner � gauche
		{"L", "./images/cards/LaserCard.png"}, // laser
		*/
	...
	{
		// nombre al�atoire entre 0 et nb de joueurs
		Random random = new Random();
		int numJoueur;
	...
	private void ExecuterProcedureAvancer() {
		...
		if(newLigTortue < 0 || newLigTortue > 7 || newColTortue < 0 || newColTortue > 7) {
			...
			if(nouvelleCase.startsWith("R")) {// un joyau gang� ! youpi !!
				ListJoueurs.get(JoueurCourant).NouveauJoyau(nouvelleCase,NbJoueur, ++NbGagnant);
				//nouvelle coordonn�es de la tortue;
				...
				//si carambolage avec une autre tortue/les deux tortues retournent � leur position initiale
				GrilleDeJeu[ligTortue][colTortue]="";
	...
	private boolean ExecuterProcedureLaser(int lig, int col) {
		...
		if(GrilleDeJeu[lig][col].startsWith("R") ) {
			//ruby / le laser est refl�chi, la tortue rebrousse chemin
			//GrilleDeJeu[ligTortue][colTortue]="";
			if(NbJoueur == 2 ) {
				//demie-tour
				FaireDemiTourTortue(JoueurCourant);
		...
		if(GrilleDeJeu[lig][col].startsWith("T")) {
			// tortue/ le laser a touch� une tortue
			String nouvelleCase=GrilleDeJeu[lig][col];
			int numJoueurTouche=0; // de la tortue touch�e
			if(NbJoueur == 2 ) {
				//demie-tour
